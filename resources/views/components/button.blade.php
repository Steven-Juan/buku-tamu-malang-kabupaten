...
@php
    ...
    $externalClasses = $attributes->get('class');
    $attributes = $attributes->except('class');
@endphp

{{-- Container dengan shadow yang lebih lembut --}}
<div
    class="button-bg-animate rounded-full p-[2px] transition duration-300 hover:scale-105 active:scale-95 shadow-md shadow-primary/10 {{ $widthClass }} {{ $externalClasses }}">
    <{{ $tag }}
        {{ $attributes->merge([
